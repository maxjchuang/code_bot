...
- `/model` now prefers an interactive Feishu card with model and reasoning selectors. `/model <model> [reasoning]` remains the direct text fallback. In both forms, it saves chat/project defaults and switches the running session when one is active. Saved selections override project model args for future `/new` and `/resume` commands.
