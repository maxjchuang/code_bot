...
describe('SessionManager', () => {
  ...
  it('caps live status chunks collected from terminal redraw output', async () => {
    ...
    const runner = new FakeCodexRunner();
    const manager = new SessionManager(sampleConfig(root), store, runner, {
      codexStatus: { quietMs: 25, liveFetchTimeoutMs: 100 },
    });
    ...
    await waitForAssertion(() => {
      expect(runner.sentMessages).toContain('status');
    });
    const redrawOutput = runner.emitOutput(sessionId, '\u001b[?2026h\u001b[35;1H•Working'.repeat(10_000));
    const modelOutput = runner.emitOutput(sessionId, '\nmodel: gpt-5.5\n');
    await redrawOutput;
    await modelOutput;
  ...
  it('caps live status chunks without resolving from a retained partial redraw fragment', async () => {
    const root = await createTmpDir();
    const store = new FileStateStore(root);
    const runner = new FakeCodexRunner();
    const manager = new SessionManager(sampleConfig(root), store, runner, {
      codexStatus: { quietMs: 5, liveFetchTimeoutMs: 250 },
    });
  ...
  it('caps live status chunks without resolving from an unterminated control fragment', async () => {
    const root = await createTmpDir();
    const store = new FileStateStore(root);
    const runner = new FakeCodexRunner();
    const manager = new SessionManager(sampleConfig(root), store, runner, {
    ...
    await runner.emitOutput(sessionId, `\u001b[${';'.repeat(32_790)}`);
    await delay(25);
